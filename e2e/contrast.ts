 ...
 *    files the node under `incomplete`. The `select` controls paint their
 *    chevron as a `url()` background layer over a flat fill, the `.meter-bar`
 *    fills are gradients, and several surface tints are `color-mix(...)` washes
 ...
 *    which is not the colour that lands on screen. Every disabled control here
 *    renders at `opacity: .45`, and the circuit stage dims un-evaluated gates
 *    the same way.
 *
 ...
 * rather than against white. A gradient is sampled at the text's own point.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. `.circuit-wrap`
 *    is an `overflow-x: auto` scroller around a comparator diagram far wider
 *    than a phone, and `.table-wrap` is another around the five-column MPC
 *    comparison table, so at 380px a lot of both is clipped away. Content
 ...
 *    is outside every ancestor's box, so the walk would find nothing behind it
 *    and fall through to white, inventing a failure. Skip it, and rely on the
 *    wider viewport where the very same element is measured for real.
 *
 ...
 *    draws two real SVG figures — the AND-gate glyph and the whole comparator
 *    circuit — and both put a `<rect>` down and then its label as a following
 *    `<text>`, so a label's backdrop is a PRECEDING SIBLING shape, not an
 *    ancestor. An ancestor-only walk would composite those labels onto the
 ...
 *    is composited first — and SVG ink is read from `fill`, not `color`.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: this page mixes accents and
     * status hues into surfaces with `color-mix()`, and the shared header bar
     * uses `color-mix(in srgb, ...)`. Chromium reports those
     ...
     * that spans its element, but a wash that fades to `transparent` partway
     * across its box, judged at its worst stop, invents backdrops that text a
     * page-height away never sits on (in one lab, accent links read 3.56:1 where
     * they actually render ~4.9:1). So each gradient is *sampled* at
     ...
     *
     * This page renders a whole comparator circuit as SVG — a node, a label and a
     * wire annotation per gate — on top of six exhibits of prose, four quizzes
     * and a five-column table. Hundreds of text nodes all walk the same handful
     * of `.section` / `.card` ancestors. Without
     ...
     * surface it happens to sit on and INVENTS failures. This page uses the idiom
     * for the comparison table's `<caption class="sr-only">` and for the spoken
     * twins of several `aria-hidden` glyphs, and `.sr-only` here is the classic
     * `clip: rect(0, 0, 0, 0)` recipe. Deliberately narrow — only a ZERO-area clip
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab ships no `<details>` at all — its exhibits
      // inject panels on click instead — but the guard is kept because it is
      // cheap, silent when unused, and the `[hidden]` narration panel behaves
      // the same way.
     ...
     * it, this page's padlock emoji (🔓/🔒) — one per garbled truth-table row and
     * one per OT box — are measured as if the glyph took the element's declared
     * `color`. An emoji paints in its own multicolour, so that ratio describes
     * nothing on screen.
     */
